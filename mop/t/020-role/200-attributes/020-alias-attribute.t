...
subtest '... simple adding an attribute alias test' => sub {
    ...
    is(
        exception { $role->alias_attribute( bar => $Bar::bar_initializer ) },
        undef,
        '... added the attribute alias successfully'
    );

    my $a = $role->get_attribute_alias('bar');
    isa_ok($a, 'mop::object');
...
subtest '... simple adding an attribute test (when %HAS is present)' => sub {
    ...
    my $a = $role->get_attribute_alias('foo');
    isa_ok($a, 'mop::object');
    isa_ok($a, 'mop::attribute');
